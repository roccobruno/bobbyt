    ...
    "from" ->emailToSent.from, "text"-> emailToSent.text,"subject"->emailToSent.subject.get).
      ...
      response =>
        response.status match {
          ...
          case 400 => println("MailGun Request fails with response 400. Check the parameters passed"); throw new Exception("MailGun request failed. Either the APIs are changed" +
